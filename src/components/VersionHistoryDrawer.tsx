      ...
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-300">
        ...
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-slate-800 text-sm">Version History & Audit</h3>
              <p className="text-xs text-slate-500">Non-destructive time-travel restore</p>
            </div>
          </div>
        ...
        {/* Notifications */}
        {successMessage && (
          <div className="mx-4 mt-3 p-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-lg flex items-center gap-2 animate-in fade-in">
        ...
        {error && (
          <div className="mx-4 mt-3 p-3 bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-lg flex items-center gap-2">
        ...
        <form onSubmit={handleCreateCheckpoint} className="p-4 border-b border-slate-200 bg-white">
          <label className="block text-xs font-semibold text-slate-700 mb-1.5 flex items-center justify-between">
            <span>Create Named Checkpoint</span>
            <span className="text-[10px] text-blue-600 font-normal">Saves to PostgreSQL</span>
          </label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 text-xs border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 hover:bg-white transition"
              disabled={isCreating}
            />
            <button
              type="submit"
              disabled={isCreating || !newVersionName.trim()}
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-semibold rounded-lg shadow-xs flex items-center gap-1 transition shrink-0"
            >
            ...
            versions.map((ver, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {idx === 0 && (
                        <span className="px-1.5 py-0.5 text-[10px] font-bold bg-blue-50 text-blue-600 rounded">
                          Latest
                        </span>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-blue-600 hover:bg-blue-50 border border-blue-200 rounded-lg transition disabled:opacity-50 shrink-0"
                ...
                <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400">
                  ...
                  <span className="font-mono">{ver.bytes} bytes</span>
                </div>
              </div>
            ))
          )}
